...
import com.notes.util.WordCloud
...
class IndividualActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun initUser() {
        userImage.setImageBitmap(User.userImageBitmap)
        userName.text = User.username // 用户名
        User.userImageBitmap?.let { changeBackground(it) }
...
//            refreshWordCloud()
//        }
        WordCloud.usedList.add("语文")
        WordCloud.usedList.add("数学")
        WordCloud.usedList.add("英语")
        WordCloud.usedList.add("物理")
        WordCloud.usedList.add("化学")
        WordCloud.usedList.add("生物")
        wordCloud.setWords(WordCloud.usedList)
    }

    private fun drawIndexTable() { // 绘制指数表
//        val data = db.getIndexData()
        val data = IntArray(112)
            ...
            takePhoto -> {
                if (resultCode == Activity.RESULT_OK) { //✔键
                    ...
                    if (bitmap != null) { // 图片不为空
                        db.changePhoto(User.userImage, bitmap)
                        User.changeUserImage(bitmap)
